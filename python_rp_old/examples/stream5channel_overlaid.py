...
# Setup parameters
frame_len = 1024
sampling_frequency = 125e3  # 125 kHz
...
plot_configs = [
    {
        ...
        'inputs': [(0, 'c', 'Ch0'), (2, 'g', 'Ch2'), (4, 'y', 'Ch4')],
        'output': (1, 'r', 'Ch1')
    },
    {
...
for row, config in enumerate(plot_configs):
    ...
    for ch_idx, color, label in config['inputs']:
        curve_time = p_time.plot(pen=color, name=f'{label} (in)')
        time_curves[(row, ch_idx)] = curve_time
        
        curve_asd = p_asd.plot(pen=color, name=f'{label} (in)')
        asd_curves[(row, ch_idx)] = curve_asd
    
    # Create curves for output
    ch_idx, color, label = config['output']
    curve_time = p_time.plot(pen=color, name=f'{label} (out)', width=2)
    time_curves[(row, ch_idx)] = curve_time
    
    curve_asd = p_asd.plot(pen=color, name=f'{label} (out)', width=2)
    asd_curves[(row, ch_idx)] = curve_asd
